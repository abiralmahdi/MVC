...
def generate_pdf(request, filename, reportID):
    ...
    output_path = os.path.join(settings.MEDIA_ROOT, "generatedReports", "Report-"+str(report.id)+'.pdf')
    ...
    # logo_path = os.path.join(settings.BASE_DIR, 'report', 'static', 'customerLogo.png')
    ...
    def add_header_footer(page_number):
        ...
        # ✅ Add user name and role at bottom right
        c.setFont("Helvetica", 10)
        signature_lines = [
            f"Report Generated by:",
            f"{report.user.name} - {report.user.role}"
        ]
        sig_y = 30  # vertical position same as page number baseline
