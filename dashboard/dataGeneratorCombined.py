...
def generate_power_meter_data(meter, start_time, num_entries, interval):
    ...
    for _ in tqdm(range(num_entries), desc=f"{meter.name} (Power)"):
        ...
        total_apparent_power = round(
            (total_active_power**2 + total_reactive_power**2) ** 0.5, 3
        )

        # Increment Energy (kWh) → Active Power × (interval hours)
        ...
        data = {
            ...
            "Energy": round(total_energy, 3),  # in kWh
        }

        readings.append(
            MeterReading(meter=meter, timestamp=current_time, data=data)
        )
        current_time += interval

    MeterReading.objects.bulk_create(readings, batch_size=1000)
    print(f"✅ Inserted {len(readings)} readings for {meter.name} (Power)")
